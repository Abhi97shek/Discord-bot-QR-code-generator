...
client.on("messageCreate", async (msg)=>{
    ...
    {
        return msg.reply("Please Enter the Command");
    }
    ...
    const size = arg[2];
    const colorPrefix = "#";
    let color = convertColor(arg[3]);
    color = color.slice(colorPrefix.length);
  

    if(cmd === "!QR" || cmd === "!qr" || cmd === "!Qr")
    {
         if(!arg[1])
        {
                return msg.reply("Please Pass the Data");
         }
        if(arg[4])
        {
